...
t_command *create_command_export_set_temp(void)
{
	...
	cmd->arguments = create_args_temp(2, (char *[]){"export", "MYVAR=hello"});
...
t_pipeline *create_pipeline_export_set_temp(void)
{
...
{
	shell->commands = create_command_export_set_temp();
	shell->pipeline = create_pipeline_export_set_temp();
}