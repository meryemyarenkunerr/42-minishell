...
// Heredoc + redirection test için mock
t_command *create_command_heredoc_redirect_temp(void)
{
	...
	cmd->heredoc_delimeter[0] = strdup("END");
...
t_pipeline *create_pipeline_heredoc_redirect_temp(void)
{
...
{
	shell->commands = create_command_heredoc_redirect_temp();
	shell->pipeline = create_pipeline_heredoc_redirect_temp();
}