...
void run_echo_tests(t_shell *shell)
{
    t_pipeline *tests[] = {
        test_echo_no_args(),
        test_echo_hello(),
        ...
        test_echo_n_hello(),
        test_echo_nn_hello(),
        test_echo_multiple_n_flags(),
        test_echo_invalid_flag(),
        test_echo_flag_middle(),
        test_echo_consecutive_n_flags(),
        test_echo_valid_then_invalid_flag(),
        test_echo_redirect_output(),
        test_echo_n_append()
    };
    ...
    const char *test_names[] = {
        ...
        "echo -nn hello",
        "echo -nnn -nn hello",
        "echo -na hello (invalid)",
        "echo hello -n world (flag middle)",
        "echo -n -nn -nnn hello world",
        "echo -nnn -na hello (valid then invalid)",
        "echo hello > output.txt",
        "echo -n hello >> append.txt"
    };
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        shell->pipeline = tests[i];

        if (!builds_commands_from_pipeline(shell))
        {
            printf("❌ Pipeline build failed\n");
            continue;
        }

        if (!setup_file_descriptors(shell))
        {
            printf("❌ FD setup failed\n");
            continue;
        }
        ...
        printf("Arguments: ");
        int j = 0;
        while (shell->commands->arguments[j])
        {
            printf("'%s' ", shell->commands->arguments[j]);
            j++;
        }
        printf("\nOutput: ");
