...
// CD .. komutu için test mock
t_command *create_command_cd_parent_temp(void)
{
	...
	cmd->cmd = strdup("cd");
	cmd->arguments = create_args_temp(2, (char *[]){"cd", ".."});
...
t_pipeline *create_pipeline_cd_parent_temp(void)
{
...
{
	shell->commands = create_command_cd_parent_temp();
	shell->pipeline = create_pipeline_cd_parent_temp();
}